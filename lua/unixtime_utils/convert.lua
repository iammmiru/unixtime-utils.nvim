...
local function parse_date(str)
  ...
  --  DD.MM.YYYY HH:MM
  --  DD.MM.YYYY HH:MM:SS
  if not str then return nil, 'empty input' end
  str = str:gsub('%s+$','')
  local d, m, y, H, M, S
  -- Full with seconds
  ...
  if not d then
    ...
    if d then H, M, S = '00','00','00' end
  end
  if not d then return nil, 'format mismatch' end
  d, m, y = tonumber(d), tonumber(m), tonumber(y)
  H, M, S = tonumber(H), tonumber(M), tonumber(S)
  if not (d and m and y and H and M and S) then return nil, 'number parse error' end
  if m < 1 or m > 12 then return nil, 'month out of range' end
  if d < 1 or d > 31 then return nil, 'day out of range' end
  if H > 23 or M > 59 or S > 59 then return nil, 'time out of range' end
  return d,m,y,H,M,S
end
...
local function compute_local_utc_offset(epoch_local_guess)
  ...
  local reconstructed = os.time({
    year = utc_table.year, month = utc_table.month, day = utc_table.day,
    hour = utc_table.hour, min = utc_table.min, sec = utc_table.sec,
...
local function resolve_epoch(d,m,y,H,M,S)
  ...
  if not local_epoch then return nil, 'os.time failed' end
  if tz == 'local' then return local_epoch end
  ...
  if tz == 'UTC' then return epoch_utc end
...
local function ensure_popup_hl()
  ...
  if not p.highlight then return end
...
if config.keymap then
  vim.keymap.set('n', config.keymap, function() Convert.open_popup() end, { desc = 'Convert human date to unix ms' })
end
